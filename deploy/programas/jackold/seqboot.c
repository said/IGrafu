...
void   doinput2(int argc, Char *argv[], int end,int wei);
...
void   help(void);// coloca por mim.
//void   trataArg(int argc, Char *argv[])
/* function prototypes */
#endif
...
void allocnew(void)
{    /* allocate memory for arrays that depend on the lenghtof theoutput sequence*/
  long i;
...
void doinput2(int argc, Char *argv[], int end,int wei)
{ // tenho q testar se o arquivo [a,f,c] sao lidos e salvos no diretorio certo.
  //estou passando como indicativo o argv[1] q eh onde salvo os saidas.

  seqboot_inputnumbers();
  allocrest();
  if (weights)
    openfile(&weightfile,argv[wei],"input weight file",
               "r",argv[wei],weightfilename);
  if (mixture){//aki
    openfile(&mixfile,argv[end],"mixture file", "r",argv[end],mixfilename);
    openfile(&outmixfile,"outmixture","output mixtures file","w",argv[1],
               outmixfilename);
    seqboot_inputaux(mixdata, mixfile);
  }
  if (ancvar){//aki
    openfile(&ancfile,argv[end],"ancestor file", "r",argv[end],ancfilename);
    openfile(&outancfile,"outancestors","output ancestors file","w",argv[1],
               outancfilename);
    seqboot_inputaux(ancdata, ancfile);
  }
  if (categories) {//aki
    openfile(&catfile,argv[end],"input category file","r",argv[end],catfilename);
    openfile(&outcatfile,"outcategories","output category file","w",argv[1],
               outcatfilename);
    inputcategs(0, sites, category, 9, "SeqBoot");
  }
  if (factors){//aki
    openfile(&factfile,argv[end],"factors file","r",argv[end],factfilename);
    openfile(&outfactfile,"outfactors","output factors file","w",argv[1],
               outfactfilename);
  }
  if (justwts && !permute){
    openfile(&outweightfile,"outweights","output weight file",
               "w",argv[1],outweightfilename);
}
  else {
    openfile(&outfile,argv[2],"output data file","w",argv[2],outfilename);
  }
  inputoptions();
  seqboot_inputdata();
}  /* doinput2 */
...
void help(){
printf("./seqboot [arquivo_entrada] [arquivo_saida] [seed] [R] [D] [J] [I] [16] [A,F,C] [E] [B] [W] [S] [X] [N] [%,P] [1] [2] [.]\n");
      printf("As letras indicam opcao do menu interativo.E todos os parametros devem ser escrito com letras maiuscula.\n\n");
      printf("[arquivo_entrada] -- indicar diretorio/arquivo para leitura do arquivo entrada -- PARAMETRO OBRIGATORIO \n");
      printf("[arquivo_saida] -- indicar diretorio/arquivo para criacao do arquivo saida -- PARAMETRO OBRIGATORIO \n");
      printf("[seed] -- semente aceitando VALOR impar -- PARAMETRO OBRIGATORIO\n");
      printf("[R] -- quantidade de replicas, aceitando VALOR ex: 100 -- PARAMETRO OBRIGATORIO \n");
      printf("[D] -- tipo da sequencia, pode ser:\n");
      printf("\t SEQUENCIA MOLECULAR:(SEQU); MORFOLOGIA DISCRETA: (MORF) ; RESTRICAO DE SITIOS: (REST); FREQUENCIA DE GENES:(FREQ) -- PARAMETRO OBRIGATORIO\n");
      printf("[J]-- modelo estatistico, pode ser:\n\tBOOTSTRAP:(BOOT); JACKKNIFE:(JACK); PERMUTE CHARACTER:(PERM); PERMUTE CHARACTER ORDER:(PORD);PERMUTE WHITH SPECIES:(PESPC); REWRITE:(REWR); -- PARAMETRO OBRIGATORIO\n");
      printf("[I] -- formato do arquivo_entrada,sendo INTERLEAVED(I) ou SEQUENCIAL(S) -- PARAMETRO OBRIGATORIO\n");
      printf( "[16] -- indica tipo da sequencia avaliada,pode ser:\n\t dna(DNA); rna(RNA);proteina(PRO) -- PARAMETRO OBRIGATORIO\n");
   printf("[A,F,C] -- arquivo de informacoes extra sobre as categorias de sitios ou fatores ou alelos, deve passar dois parametro:\n\to primeiro a palavra chave: ALL ou CAT ou FAC, \n\to segundo o diretorio/NOME_ARQUIVO \n\t--  PARAMETRO NAO OBRIGATORIO\n");
      printf("[E] --  informa as enzimas no arquivo entrada, deve passar no parametro a palavra ENZIMA -- PARAMETRO NAO OBRIGATORIO\n");     
      printf("[B] -- tamanho do bloco que divide a sequencia,deve passar dois parametro:\n\t o primeiro a palavra BLOCO,\n\t o segunto o tamanho (valor)que sera dividido as sequencias\n -- PARAMETRO NAO OBRIGATORIO\n");
      printf("[W]-- arquivo que contem os pesos do caracter, deve passar dois parametros:\n\t o primeito a palavra WEIGHT,\n\t o segundo o diretorio/NOME_ARQUIVO\n -- PARAMETRO NAO OBRIGATORIO \n");
      printf("[S] -- produce multiple output files, deve passar como parametro o nome:JUSTW( just weight) ou OUTD(out data) -- PARAMETRO NAO OBRIGATORIO\n");
      printf("[X] -- arquivo_mixture, deve passar dois parametro \n\t o primeiro a palavra MIX, \n\t o segundo o  iretorio/NOME_ARQUIVO\n\t--PARAMETRO NAO OBRIGATORIO \n");
      printf("[N] -- arquivo_ancestral, deve passar dois parametro \n\t o primeiro a palavra ANC, \n\t o segundo o diretorio/NOME_ARQUIVO\n\t -- PARAMETRO NAO OBRIGATORIO \n");
	printf("[%] -- indicar a porcentagem das amaostras, deve passar dois parametro:\n\t o primeiro a palavra FRACAO,\n\t o segundo o valor da porcentagem das amostras --PARAMETRO NAO OBRIGATORIO\n");
      printf("[P] -- formato saida para rescrever o arquivo,deve passar dois parametros\n\t o primeiro a palavra RESC, \n\t o segundo uma das palavras: PHY(formato phylip),NEX(formato nexus),XML(formato xml) -- PARAMETRO NAO OBRIGATORIO\n");
      printf("[1] -- escrever os dados de saida, deve passar como parametro a palavra PRINT -- PARAMETRO NAO OBRIGATORIO\n");
      printf("[2] -- indicar execucao,deve passar como parametro a palavra RUN -- PARAMETRO NAO OBRIGATORIO\n");
      printf("[.] -- usar o dot-differencing, deve passar como parametro a palavra DIFFER -- PARAMETRO NAO OBRIGATORIO\n");
	exit(0);
	}
...
{  /* Read in sequences or frequencies and bootstrap or jackknife them */
  ...
  argv[0] = "SeqBoot";
#endif
  long dd,inseed0,inseed=3;
  int cont=9, end=1,wei;
float db;
  init(argc,argv);
  ibmpc = IBMCRT;
  ansi = ANSICRT;

  if(argc > 1){// entra com argumentos... 
        if ( strcmp(argv[1],"help" )==0)// imprime tela de ajuda
          help();
        else{
          if ( argc < 8 ){// quantidade de parametro errado.
	 printf("Quantidade INSUFICIENTE de parametros...\n");
	 printf("Deve conter todos descrito na ajuda como OBRIGATORIO, na ORDEM, e CERTOS como apresentado na ajuda.\nOs parametros NAO obrigatorios escritos errados sao desconsiderados\nDigete 'help' para mais informacoes\n");
	   exit(0);
	    }}
// default values
	  data = seqs;
...
// trata opçao obrigatoria na ordem
dd = atol(argv[3]);//seed
    if ( (dd<=0)||(dd%2==0) )
     { printf(" A semente tem que ser numero impar\n");
        exit(0); 
     } else 
       inseed=dd;
...
printf("valor nao aceito(R),o valor deve ser positivo e maior que 1\n");
                    exit(0);
           }

if(strcmp(argv[5],"SEQU")==0) //d
    data = seqs;         // trata tipo de sequencia
    else {
       if(strcmp(argv[5],"REST")==0)
            data =restsites;
        else {
            if(strcmp(argv[5],"FREQ")==0)
              data = genefreqs;
              else {
                 if(strcmp(argv[5],"MORF")==0)
                 data=morphology;
                else{
                    printf("Tipo de sequencia nao aceito(D).Escolha uma das opcoes predefinida\n");
                    exit(0);
                }}}}
if ( strcmp(argv[6],"BOOT" )==0){
        bootstrap = true;
        jackknife = false;
   }
   else{
      if ( strcmp(argv[6],"JACK" )==0)//j
        { bootstrap = false;
         jackknife = true;
         }else{
          if ( strcmp(argv[6],"PSPEC" )==0)
          {   bootstrap = false;
              lockhart = true;
          } else{
             if ( strcmp(argv[6],"PORD" )==0)
               { ild = true;
                 bootstrap = false; 
             }else{
               if ( strcmp(argv[6],"REWR" )==0)
                  bootstrap = false; //rewrite
               else{
                  if ( strcmp(argv[6],"PERM" )==0)
                  {   bootstrap = false;
                      permute= true;
                  }
                  else{
               printf("Parametro nao aceito (J).Escolha uma das opcoes predefinida.\n");
                    exit(0);
                   }  }}}}}

  if ( strcmp(argv[7],"S")==0)//i
     interleaved =false;
  else{ 
    if ( strcmp(argv[7],"I")!=0)
    { printf("Opcao nao aceita (I).Escolha uma das opcoes predefinida.\n");
      exit(0);
    }}

if  ( strcmp(argv[8],"DNA" )==0)//16
seq= rna;
else{
    if  ( strcmp(argv[8],"RNA" )==0)
     seq=dna;
   else{
if  ( strcmp(argv[8],"PRO" )==0)
    seq=protein;
else{
printf("Opcao nao aceita(16).Escolha uma das opcoes predefinida e verifique se as letras estao todas em maiusculas\n");
                    exit(0);
}}}

// tratar opçoes nao obrigatoria

while(cont<argc){
     if ( data=restsites && ( strcmp(argv[cont],"ENZIMA" )==0))
       enzymes= true;// ler enzimas
     if ( strcmp(argv[cont],"JUSTW" )==0)//S
      justwts = true;
     if ( strcmp(argv[cont],"OUTD" )==0)//S
      justwts = false;
     if  ( strcmp(argv[cont],"PRINT" )==0)//print data
         printdata= true;
     if  ( strcmp(argv[cont],"RUN" )==0)//print processo
          progress= true;
     if  (( strcmp(argv[cont],"DIFFER" )==0)&& printdata)//dot-differ 1+DOT
        dotdiff= true;
   //  if((cont+1)<argc){// exige 2 parametros casado.
           if(strcmp(argv[cont],"BLOCO" )==0){
              cont=cont++;
              dd = atol(argv[cont]);// b
              if ( dd>1 )
                blocksize=dd;
              else{
                printf("parametro errado (valor do bloco).\n");
                exit(0);
                  }
            }
          if ( strcmp(argv[cont],"FACAO" )==0)//%
               {  cont++;
                  regular=false;
                   db = atof(argv[cont]);
                   if( !((db>1.0) && (bootstrap||jackknife)) )
                      { printf("Opcao nao acaita(%). Valor negativo ou igual a zero.\n");
                      exit(0);
                }else{
                 regular = false;
                 fracsample =db;
                 fracsample = fracsample/100.0;
                        ...
                        if(strcmp(argv[cont],"RESC" )==0)//P
                        {   cont++;
                        if ( (data == seqs)||(data==morphology))
                         {if(strcmp(argv[cont],"PHY" )==0)
                        {   xml= false;
                           nexus = false;
                        }else{
                      if(strcmp(argv[cont],"XML" )==0)
                          xml = true;
                         else{
                        if(strcmp(argv[cont],"NEX" )==0)
                         nexus = true;
                 }}}else{
                   printf("parametro %s nao reconhecido na opcao RESC",argv[cont]);
                    }}else{
               if ( strcmp(argv[cont],"WEIGHT" )==0)//W
                 { if(filexists(argv[++cont])){
                     weights = true;
                     wei=cont;
                    }else{
                      printf("Arquivo WEIGHT passado no parametro  %d nao existe\n",cont);
                      exit(0);
                       }
                 }else{
                  if(strcmp(argv[cont],"FAC")==0)//f
                   {   if(filexists(argv[++cont])){
			factors= true;// se ler factors
                        end=cont;
                   } else{
                      printf("Arquivo FACTOR passado no parametro  %d nao existe\n",cont);
                      exit(0);
                       }
          }else{
            if(strcmp(argv[cont],"MIX")==0)//x
             { if(filexists(argv[++cont])){
               mixture= true;// se ler mixture
               end=cont;
                   } else{
                      printf("Arquivo MIXTURE passado no parametro  %d nao existe\n",cont);
                      exit(0);
                       }
         }else{
            if(strcmp(argv[cont],"ANC")==0)//n
             {if(filexists(argv[++cont])){
               ancvar= true;// se ler ancestral
               end=cont;
                   } else{
                      printf("Arquivo ANCESTRAL passado no parametro  %d nao existe\n",cont);
                      exit(0);
                       }
             }else{
                if ( strcmp(argv[cont],"ALL" )==0) //all
                 { if(filexists(argv[++cont])){
                    all=true;//se ler all
                     end=cont ;
                   } else{
                      printf("Arquivo ALLELOS passado no parametro  %d nao existe\n",cont);
                      exit(0);
                       }
              }
             else{
                if ( data==seqs&& strcmp(argv[cont],"CAT" )==0)//cat
                   {cont++;
                     printf("verificando arquivo cat\n");
                     if(filexists(argv[cont])){
                       end=cont ;
                      printf("%s\n",argv[end]);
                     categories = true;
                    } else{
                      printf("Arquivo CATEGORIES passado no parametro  %d nao existe\n",cont);
                      exit(0);
                      }
               }
             }}}}}}
               }
//printf("valor de cont= %d e valor argc= %d\n", cont, argc);
cont++;
}
 ...
 if (bootstrap || jackknife || permute || ild || lockhart)
    initseed2(&inseed, &inseed0, seed);

openfile(&infile, argv[1], "input file", "r", argv[1], infilename);
doinput2(argc, argv, end,wei);
} else{
  openfile(&infile, INFILE, "input file", "r", argv[0], infilename);  
  doinput(argc, argv);
}// nao entrou linha d comando
// codigo comum.
  bootwrite();
  FClose(infile);
  if (weights)
