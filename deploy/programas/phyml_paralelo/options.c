...
option *Get_Input(int argc, char **argv, int myid)
{
  ...
  switch (argc)
      ...
      if (isdigit((int)argv[N_DATATYPE][0]))
	{
	  if (atoi(argv[N_DATATYPE])==0 && argc-1==N_NB_PARAMS_DNA)
	    Get_Input_CommandLine_DNA(input, argc, argv, myid);
	  else if (atoi(argv[N_DATATYPE])==1 && argc-1==N_NB_PARAMS_AA)
	    Get_Input_CommandLine_AA(input, argc, argv, myid);
	  else
	    Usage();
	}
      else
...
void Get_Input_CommandLine_Common(option *input, int argc, char **argv, int myid)
{
  char* p;
  ...
  sprintf(input->phyml_stat_file + (int)strlen(input->phyml_stat_file),"_%d",myid);
  ...
  if(!input->mod->bootstrap)
  ...
  else
    {
      ...
      sprintf(r+(int)strlen(r),"_%d",myid);
...
/*Attention phyml compil�avec les symboles de compilation EVOLVE et OPTIMIZ n'est utilisable qu'en mode interactif */
void Get_Input_CommandLine_DNA(option *input, int argc, char **argv, int myid)
{
  char* p;
  ...
  Get_Input_CommandLine_Common(input, argc-1, argvbis, myid); /*argv must not change*/
...
/*Attention phyml compil�avec les symboles de compilation EVOLVE et OPTIMIZ n'est utilisable qu'en mode interactif */
void Get_Input_CommandLine_AA(option *input, int argc, char **argv, int myid)
{
  char* p;

  Get_Input_CommandLine_Common(input, argc, argv, myid);
