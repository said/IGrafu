...
{
  errno = 0;
  if(*s != ',' && *s != ')' && *s != ':' && *s != '*' && (!strtod(s,(char **)NULL) && errno))
    {
      printf("\n ! Tree bad format !\n");
      exit(1);
    }
...
char *Write_Tree(arbre *tree)
{
  ...
  /*printf("valor de i = %d valor de noutu= %d\n", i, tree->n_otu); */
...
/*   For(site,alldata->crunch_len) printf("%1.0f",alldata->wght[site]); */
  ...
  if(!input->mod->datatype){
    ...
    /*printf("executou get_base_freqs\n");*/
  }
  else
...
allseq *Make_Seq(int n_otu, int len, char **sp_names)
{
  ...
  alldata->pospatt               = (int **)mCalloc(len,sizeof(int *));
...
allseq *Copy_CData(allseq *ori, model *mod)
{
  ...
  For(i,ori->init_len)
    new->pospatt[i] = (int *)mCalloc((int)ori->wght[i],sizeof(int));
  ...
  For(j,ori->init_len) For(i,ori->wght[j]) new->pospatt[j][i] = ori->pospatt[j][i];
...
void Bootstrap(arbre *tree, int myid, int numprocs)
{
  int *site_num, n_site;
  int replicate,j,k;
...
if (myid == 0){
  ...
  matriz = (double **)mCalloc(tree->mod->bootstrap,sizeof(double *));
  for (i=0; i<tree->mod->bootstrap; i++){
    matriz[i] = (double *)mCalloc(boot_data->init_len,sizeof(double));
  }
  ...
  For(replicate,tree->mod->bootstrap)
    {
      //For(j,boot_data->crunch_len) boot_data->wght[j] = .0;
       For(j,boot_data->crunch_len) matriz[replicate][j] = .0;
	...
	//  boot_data->wght[site_num[position]] += 1.;
	  matriz[replicate][site_num[position]] += 1.;
	  init_len++;
	}
     ...
     //For(replicate,5)
      {
        For(j,boot_data->crunch_len) boot_data->wght[j] = .0;
        For(j,boot_data->crunch_len)
	 {
        boot_data->wght[j] = matriz[replicate][j];
         }
...
/*      for(i=0;i<2*tree->n_otu - 3;i++){
            printf("para tree:valor score %d do edge %d\n",tree->t_edges[i]->bip_score,i);
	    }
       for(i=0;i<2*boot_tree->n_otu - 3;i++){
                       printf("para boot_tree: valor score %d do edge %d\n",boot_tree->t_edges[i]->bip_score,i);
      }
 */     
...
/* 	rf += tree->t_edges[j]->bip_score; */
   ...
   } /*fecha o segundo laço de replicate*/
   
   
      
   
   
  For(i,tree->mod->bootstrap) Free(matriz[i]);
  Free(matriz);
  /*para liberar espaço */
